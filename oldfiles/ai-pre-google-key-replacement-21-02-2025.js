...
async function detectLanguage(text) {
  const apiKey = "AIzaSyAKXU_f0p1vIWcDMU4bc71_SM4eufqDYLA";
...
const generateResponse = async (incomingChatLi) => {
  ...
  try {
    ...
    assistantResponse = assistantResponse.replace(/【4:\d+†source】/g, '');
...
async function createThread(userMessage) {
  try {
    ...
    return 'Sorry, there was an issue creating the thread.';
  }
}
...
async function addMessageToThread(threadId, userMessage) {
  ...
  try {
    ...
    return 'Sorry, there was an issue adding your message.';
  }
}
