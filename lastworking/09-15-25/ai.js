...
const OPENAI_API_KEY = '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';
const ASSISTANT_ID = 'asst_LyEOczS6GfHNK5qvAECgLagg';
...
function getGoogleVoice(languageCode) {
  return voices[languageCode] || "en-US-Chirp3-HD-Leda";
}
...
const generateResponse = async (incomingChatLi) => {
  ...
  try {
    ...
    await addMessageToThread(threadId, userMessage, transcriptLanguage);
    await runThread();
    assistantResponse = await pollThreadMessages();
    assistantResponse = assistantResponse.replace(/【4:\d+[^\]]*】/g, '');
...
// Function to create a new thread for the assistant
...
async function addMessageToThread(threadId, userMessage, detectedLanguage) {
  if (!threadId) {
    return "The conversation hasn't been started yet. Please initiate it first.";
  }

  const languageInstruction = `Please respond in ${detectedLanguage}.`;
  const modifiedMessage = `${languageInstruction}\n\n${userMessage}`;
  ...
  try {
    const response = await fetch(`${BASE_URL}/threads/${threadId}/messages`, {
      ...
      body: JSON.stringify({ role: 'user', content: modifiedMessage })
    });
...
// Function to run the thread (get the assistant's updated response)
...
async function getThreadMessages() {
  ...
  try {
    ...
    console.error('Error running the thread:', error);
    return 'Sorry, there was an issue running the thread.';
  }
}
...
async function pollThreadMessages() {
  if (!threadId) {
    console.error("The conversation hasn't been started yet. Please initiate it first.");
    return null;
  }
  ...
  try {
    do {
      data = await getThreadMessages(); // Call the function
      console.log(data);

      responseText = data.data
        .filter(item => item.role === "assistant") // Filter items where role is 'assistant'
        .flatMap(item => item.content) // Get the 'content' array from each item
        .filter(content => content.type === "text") // Ensure the content type is 'text'
        .map(content => content.text.value) // Extract the 'text.value'
      ...
      await timeout(1000); // Wait for 1 second before retrying
    } while (true);
  } catch (error) {
    console.error("Error polling thread messages:", error);
    return null;
  }

  return data; // Return the data once the condition is met
}
