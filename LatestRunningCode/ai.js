...
const OPENAI_API_KEY = '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';
...
async function getAudio(requestBody) {
  try {
    ...
    try {
      ...
      throw new Error("Invalid JSON returned from Google TTS");
    }

    console.log("✅ Parsed Response from API:", data);

    if (!data.audioContent) {
      console.warn("⚠️ No audioContent in response! Full response:", data);
      throw new Error("No audio content returned from Google TTS API");
    }

    console.log("🔊 audioContent length (base64):", data.audioContent.length);
