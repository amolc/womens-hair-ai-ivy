...
async function detectLanguage(text) {
  try {
    ...
    console.log("detectLanguage response:", data);
    ...
    console.error("Error calling backend language detection API:", error);
...
function buildRequestBody(message) {
  ...
  return {
    ...
    voice_name: "en-US-Chirp3-HD-Leda"  // Adding Leda voice parameter
  };
}
...
async function getAudio(requestBody) {
  try {
    ...
    const data = await response.json();
    console.log("Response from backend TTS API:", data);

    // Check if the response was successful
    if (data.success) {
      return data.audioContent;
    } else {
      throw new Error(data.error || "TTS API returned an error");
    }
  } catch (error) {
...
async function transcribeAudio(audioBlob) {
  ...
  try {
    ...
    // Note: Backend doesn't return language info, so we'll use default
    // transcriptLanguage = data.language; // Not available from backend
...
async function convertToWebm(audioBlob) {
  ...
  const response = await fetch(api_url + 'convert-to-webm', {
    method: 'POST',
    body: formData
  });
